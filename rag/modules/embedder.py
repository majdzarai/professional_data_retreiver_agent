        ...
        if self.model_provider == "sentence_transformers":
            ...
            try:
                ...
                if self.model_name.startswith("local:"):
                    ...
                    # For local models, we'll load it during initialization
                    try:
                        ...
                        logger.error(f"Error loading local Nomic model: {e}")
                        self.model = None
                else:
                ...
                logger.warning("Nomic not installed. Run: pip install nomic")
                self.model = None
    ...
    def embed_text(self, text: str) -> np.ndarray:
        ...
        if self.model_provider == "sentence_transformers":
            ...
            try:
                ...
                # Handle local vs API models differently
                if isinstance(self.model, dict) and self.model.get("type") == "local":
                    ...
                    # Use Nomic's local embedding functionality
                    result = nomic.embed(texts=[text], model_name=local_path, use_local=True)
                    return np.array(result[0])
                else:
                    # For API models, use the standard API call
                    model_name = self.model.get("name") if isinstance(self.model, dict) else self.model_name
                    result = nomic.embed(texts=[text], model_name=model_name)
                    return np.array(result[0])
            except Exception as e:
    ...
    def _batch_embed(self, texts: List[str]) -> List[np.ndarray]:
        ...
        if self.model_provider == "sentence_transformers":
            ...
            try:
                ...
                # Handle local vs API models differently for batch processing
                if isinstance(self.model, dict) and self.model.get("type") == "local":
                    # For local models, use the local path
                    local_path = self.model.get("path")
                    # Use Nomic's local embedding functionality for batch processing
                    results = nomic.embed(texts=texts, model_name=local_path, use_local=True)
                    return [np.array(emb) for emb in results]
                else:
                    # For API models, use the standard API call
                    model_name = self.model.get("name") if isinstance(self.model, dict) else self.model_name
                    results = nomic.embed(texts=texts, model_name=model_name)
                    return [np.array(emb) for emb in results]
            except Exception as e:
